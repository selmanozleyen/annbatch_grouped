...
Uses anndata_rs.permute (scatter engine) to write a new store containing
only the first --n_rows observations. Runs out-of-core -- the full dataset
is never loaded into RAM.

Usage:
    python scripts/truncate_zarr.py --src /path/to/input.zarr --dst /path/to/output.zarr --n_rows 10000000
    python scripts/truncate_zarr.py --src /path/to/input.zarr --n_rows 5000000 --memory_limit 4294967296
"""
...
def main(
    ...
    try:
        anndata_rs.permute(str(src_path), str(dst_path), indices, **scatter_kwargs)
