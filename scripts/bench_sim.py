...
def _configure_zarr_backend(zarr_backend: str) -> None:
    if zarr_backend == "zarr-python":
        zarr.config.set(
            {
    ...
    elif zarr_backend == "zarrs-python":
        try:
            import zarrs  # noqa: F401
        except ImportError as exc:
            raise click.ClickException(
                "zarrs-python backend requested, but the `zarrs` package is not importable."
            ) from exc
        zarr.config.set(
            {
        ...
        raise click.ClickException(f"Unsupported zarr backend: {zarr_backend!r}")
    os.environ["ANNBATCH_ZARR_BACKEND"] = zarr_backend


def _active_zarr_backend() -> str:
    return os.environ.get("ANNBATCH_ZARR_BACKEND", DEFAULT_ZARR_BACKEND)
...
@click.option(
    ...
    default=None,
    help=(
        "Codec backend to use. Defaults to ANNBATCH_ZARR_BACKEND or zarr-python. "
        "zarrs-python enables codec_pipeline.strict to fail instead of falling back."
    ),
)
...
def main(
    ...
    n_batches, max_seconds, warmup, repeats, seed, zarr_backend, cpu_constraint, repeat_index,
    ...
    if zarr_backend is None:
        zarr_backend = os.environ.get("ANNBATCH_ZARR_BACKEND", DEFAULT_ZARR_BACKEND)
    if zarr_backend not in ZARR_BACKENDS:
        raise click.ClickException(f"--zarr_backend must be one of {ZARR_BACKENDS}")
    _configure_zarr_backend(zarr_backend)
    ...
    print(f"  zarr_backend:    {zarr_backend}")
